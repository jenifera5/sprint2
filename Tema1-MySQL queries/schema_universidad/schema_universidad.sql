...
-- ej 5
 ...
 select nombre,cuatrimestre,curso,id
 from asignatura
 ...
 -- ej 7
 select 
	a.nombre as  'Nombre de la asignatura' ,
    ce.anyo_inicio as 'Año de inicio',
    ce.anyo_fin as 'Año de fin'
from asignatura a 
join alumno_se_matricula_asignatura al on a.id= al.id_asignatura
join curso_escolar ce  on al.id_curso_escolar = ce.id 
join persona p on al.id_alumno = p.id 
...
where g.nombre = 'Grado en Ingeniería Informática (Plan 2015)';
 -- ej 9
 select distinct
    p.nombre as 'Nombre del alumno/a',
...
select 
 ...
 -- ej 14
 select 
 nombre as 'nombre de la asignatura'
 from asignatura
...
where a.id is null;
 -- ej 16
 select count(*) as 'Número total de alumnos'
 from persona
 where tipo='alumno';
 
 -- ej 17
 select count(*) as 'alumnos que nacieron el 1999'
from persona
where tipo='alumno'
and year(fecha_nacimiento)=1999;
 ...
 select 
	d.nombre as 'Nombre del departamento',
	count(pr.id_profesor) as 'Número de profesores/as'   
from departamento d
...
order by count(pr.id_profesor)desc;
 ...
 select
    ...
    count(a.id) as 'Número de asignaturas'
from grado g
